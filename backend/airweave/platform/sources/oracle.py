...
# Mapping of Oracle types to Python types
ORACLE_TYPE_MAP = {
    "number": int,
    "integer": int,
    "int": int,
...
@source(
    ...
    auth_config_class="OracleAuthConfig",
    ...
    async def _connect(self) -> None:
        ...
        if not self.pool:
            try:
                ...
                dsn = (
                    f"(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST={host})(PORT={self.config['port']}))"
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cursor:
                ...
                columns_query = """
                    ...
                    ORDER BY COLUMN_ID
                """
                await cursor.execute(columns_query, [schema.upper(), table.upper()])
                ...
                """
                await cursor.execute(pk_query, [schema.upper(), table.upper()])
                primary_keys = [row[0] for row in await cursor.fetchall()]

                # Build column metadata
                column_info = {}
                for col in columns:
                    oracle_type = col[1].lower()
                    python_type = ORACLE_TYPE_MAP.get(oracle_type, Any)
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cursor:
                query = """
                    ...
                    WHERE OWNER = :1
                """
                await cursor.execute(query, [schema.upper()])
