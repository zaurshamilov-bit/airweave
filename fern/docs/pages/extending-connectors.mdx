...
Here's an example of a custom source connector implementation. In this example, we're fetching data from the Studio Ghibli API and yielding it as chunks. These chunks are then used to process into your destinations.
...
class GhibliSource(BaseSource):
    ...
    This connector fetches film data from the Studio Ghibli API and yields it as chunks.
    Each chunk represents a Ghibli film with its details.
    """
    ...
    async def generate_chunks(self) -> AsyncGenerator[BaseChunk, None]:
        """Generate chunks from the Ghibli API.
        
        Each chunk contains information about a Studio Ghibli film.
        """
