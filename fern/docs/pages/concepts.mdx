...
support a [wide range of sources](https://github.com/airweave-ai/airweave/tree/main/backend/app/platform/sources) to ensure you can connect all your valuable data.
...
Chunks are the fundamental building blocks that power Airweave's search
...
<Note>
Chunks are the smallest unit of data in Airweave. Every chunk has an **entity_id** and belongs to a **sync job**, which in turn belongs to a <a href="#synchronization">sync</a>.
During ingestion, chunks are hashed and version-tracked in the Airweave database. This allows for efficient deduplication and change detection.
</Note>

#### Core Structure

Every chunk in Airweave inherits from a base structure that ensures consistency and searchability:

<CodeBlock title="Base Chunk Structure">
```python 
...
These chunks are extended by platform-specific chunks:

<CodeBlock title="Slack Message Chunk">
```python
...
## Synchronization
A syncrhonization in Airweave orchestrates sync jobs and scopes your data in the target destination. 
...
Refer to the [Sync API Reference](/api-reference/sync/create-sync) for more information on how to trigger syncs via API.
...
<Frame background="subtle" caption="White Labeling Authentication">
  <img 
    src="../assets/images/white-label-overview.png" 
    alt="White Labeling Overview"
  />
</Frame>

Refer to the [White Labeling](/docs/white-labeling) documentation for more information on how to set up white labeling.
