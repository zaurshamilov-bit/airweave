---
title: "Postgresql"
description: "Postgresql integration with Airweave"
---

{/* AUTO-GENERATED CONTENT START */}



<div className="connector-header" style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
  <img src="icon.svg" alt="Postgresql logo" width="48" height="48" className="connector-icon" />
  <h1 style={{ margin: 0 }}>Postgresql</h1>
</div>

## Configuration

PostgreSQL source connector integrates with PostgreSQL databases to extract and synchronize structured data
from database tables. It uses dynamic schema introspection to create appropriate entity classes
and provides comprehensive access to relational data with proper type mapping and relationships.

<Card
  title="View Source Code"
  icon="brands github"
  href="https://github.com/airweave-ai/airweave/tree/main/backend/airweave/platform/sources/postgresql.py"
>
  Explore the Postgresql connector implementation
</Card>

### Authentication

This connector uses a custom authentication configuration.

<Card
  title="Authentication Configuration"
  className="auth-config-card"
  style={{ backgroundColor: 'rgba(0, 0, 0, 0.1)', padding: '16px', marginBottom: '24px' }}
>

PostgreSQL authentication configuration.
<ParamField
  path="host"
  type="str"
  required={true}
>
  The host of the PostgreSQL database
</ParamField>
<ParamField
  path="port"
  type="int"
  required={true}
>
  The port of the PostgreSQL database
</ParamField>
<ParamField
  path="database"
  type="str"
  required={true}
>
  The name of the PostgreSQL database
</ParamField>
<ParamField
  path="user"
  type="str"
  required={true}
>
  The username for the PostgreSQL database
</ParamField>
<ParamField
  path="password"
  type="str"
  required={true}
>
  The password for the PostgreSQL database
</ParamField>
<ParamField
  path="schema"
  type="str"
  required={false}
  default="public"
>
  The schema of the PostgreSQL database
</ParamField>
<ParamField
  path="tables"
  type="str"
  required={false}
  default="*"
>
  Comma separated list of tables to sync. For example, 'users,orders'. For all tables, use '*'
</ParamField>
</Card>

### Configuration Options

This connector does not have any additional configuration options.





{/* AUTO-GENERATED CONTENT END */}
