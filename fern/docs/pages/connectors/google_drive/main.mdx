...
Google Drive source connector integrates with the Google Drive API to extract and synchronize files from both
personal Google Drive (My Drive) and shared drives. It supports downloading and processing files
...
<ParamField
  ...
  default="[]"
>
  List of file/folder paths or patterns to exclude from synchronization. Examples: '*.tmp', 'Private/*', 'Confidential Reports/'. Separate multiple patterns with commas.
</ParamField>
</Card>
