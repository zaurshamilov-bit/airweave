...
<Accordion title="GmailMessageDeletionEntity">
...
| Field | Type | Description |
|-------|------|-------------|
| message_id | str | The Gmail message ID that was deleted |
| thread_id | str | None | Thread ID (optional if not provided by change record) |
| deletion_status | str | Status indicating the message was removed (e.g., 'removed') |
