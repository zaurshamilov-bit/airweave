...
At the core of Airweave is our FastAPI server, which handles all API requests and coordinates system operations. We provide both [REST APIs](/api-reference) and dedicated [SDKs](/docs/sdk), making it easy to integrate Airweave into your existing agent stack.
...
**PostgreSQL**: Serves as our primary database, handling user data, configurations, chunk hashes and metadata. This does not host the actual chunks, but rather the metadata to track the chunks. Generally, you'll not need to interact with this database directly.
